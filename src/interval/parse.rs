...
use crate::{duration::parse::parse_relative_duration, parser::take_n_digits, Interval};
...
fn parse_start_and_duration(i: &[u8]) -> IResult<&[u8], Interval> {
    let (i, date) = parse_date(i)?;
    let (i, _) = tag(b"/")(i)?;
    let (i, duration) = parse_relative_duration(i)?;

    Ok((i, Interval::from_start(date, duration)))
}

fn parse_start_and_end(i: &[u8]) -> IResult<&[u8], Interval> {
    let (i, start) = parse_date(i)?;
    let (i, _) = tag(b"/")(i)?;
    let (i, end) = parse_date(i)?;

    Ok((i, Interval::with_dates(start, end)))
}

pub fn parse_interval(i: &[u8]) -> IResult<&[u8], Interval> {
