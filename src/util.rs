...
// TODO: Add fiscal year to all of these calculations
...
pub fn end_of_quarter(d: &NaiveDate) -> NaiveDate {
  addition::add_quarter_duration(*d).pred()
}
...
pub fn end_of_biweek(d: &NaiveDate) -> NaiveDate {
  addition::add_biweek_duration(beginning_of_biweek(d)).pred()
}
...
mod tests {
  ...
  fn test_add_month_quickcheck(d: NaiveDateWrapper) {
    addition::add_month_duration(d.0);
  }

  #[ignore = "broken af"]
  #[quickcheck]
  fn test_add_quarter_quickcheck(d: NaiveDateWrapper) {
    addition::add_quarter_duration(d.0);
  }
