...
fn parse_duration_chunk(input: &[u8]) -> IResult<&[u8], Unit> {
    match tuple((take_signed_digits, one_of("YMWD")))(input)? {
        (i, (amt, u)) => match u {
...
pub fn parse_relative_duration(input: &str) -> IResult<&[u8], RelativeDuration> {
    let (leftover, units) = preceded(tag(b"P"), count(parse_duration_chunk, 4))(input.as_bytes())?;
...
mod tests {
    ...
    fn test_parse_duration_chunk_weeks() {
        let (_input, duration) = parse_duration_chunk("-1M".as_bytes()).unwrap();
